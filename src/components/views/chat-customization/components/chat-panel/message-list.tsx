...
function MessageMetadata({ metadata }: MessageMetadataProps) {
  ...
  return (
    <div className="space-y-2 pt-2 border-t border-border/20">
      ...
      {cssChanges && cssChanges.length > 0 && (
        <div className="space-y-1">
          <div className="text-xs font-medium opacity-80 flex items-center gap-2">
            Applied Changes:
            <span className="text-green-400 text-xs">✓ Applied to page</span>
          </div>
          <div className="space-y-1">
            {cssChanges.map((change, index) => (
              <div key={index} className="text-xs bg-background/20 rounded px-2 py-1">
                ...
                {change.confidence < 0.8 && <span className="ml-2 text-yellow-400">⚠</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Processing time */}
      {processingTime && <div className="text-xs opacity-50">Processed in {processingTime}ms</div>}
    </div>
  );
}
