...
export const VisualPreferencesView: React.FC = () => {
  ...
  const handlePromptDialogOpen = (open: boolean) => {
    if (open) {
      // Always refresh the default prompt to show current state (including CSS)
      const prompt = domAnalysisService.getDefaultPrompt(cssContent || undefined);
  ...
  const analyzeCurrentPage = async () => {
    ...
    // Clear existing UI state but preserve CSS
    console.log('🧹 Clearing previous analysis state (preserving CSS)...');
    setElements([]);
    setPreferences({});

    // DON'T clear CSS content - preserve it for context and continuity
    console.log('✨ Preserving existing CSS content for analysis context:', {
      hasExistingCSS: !!cssContent,
    ...
    try {
      ...
      if (cssContent) {
        addLog('📝 Including existing CSS in analysis for better context...', 'info');
      }

      // Use the new LLM-based analysis with existing CSS context
      const analysis = await domAnalysisService.analyzeDOM(
        customPrompt || undefined,
        cssContent || undefined // Pass existing CSS for LLM context
      );
      ...
      if (analysis.elements.length > 0) {
        ...
        if (cssContent) {
          addLog('🔄 Existing CSS preserved and included in new controls', 'info');
        }
      } else {
  ...
  const renderPreferenceControl = (
    ...
    // Enhanced onChange handler with debugging
    const handleChange = (newValue: PreferenceValue) => {
      console.log(`🔄 Preference change triggered for ${element.id}:${option.id}`, {
        oldValue: currentValue,
        newValue,
        type: option.type,
      });
      updatePreference(element.id, option.id, newValue);
    };
    ...
    return uiGeneratorService.renderDynamicControl(
      ...
      option,
      currentValue,
      handleChange, // Use the enhanced handler
      resetPreference,
      uniqueKey
    );
  };
