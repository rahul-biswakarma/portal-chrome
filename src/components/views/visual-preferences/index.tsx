...
export const VisualPreferencesView: React.FC = () => {
  ...
  const analyzeCurrentPage = async () => {
    ...
    // IMPORTANT: Clear CSS content both in state and applied to page
    console.log('🧹 Clearing existing CSS content...');
    setCssContent(''); // Clear CSS editor content

    // Also try to remove any applied CSS from the page
    try {
      ...
      if (tab[0]?.id) {
        await chrome.scripting.executeScript({
          ...
          func: () => {
            // Remove any existing portal customization styles
            const existingStyles = document.querySelectorAll('style[data-portal-customizer]');
            existingStyles.forEach(style => style.remove());

            // Remove any styles with Visual Customizer comments
            const allStyles = document.querySelectorAll('style');
            allStyles.forEach(style => {
              if (
                style.textContent &&
                style.textContent.includes('Generated by Visual Customizer')
              ) {
                style.remove();
              }
            });

            console.log('🧹 Cleared existing page styles');
          },
        });
      }
    } catch (error) {
      console.warn('Could not clear existing page styles:', error);
    }
